...
def JsonLen(DataA, DataB, loguser):
    loguser.debug('开始判断长度，响应返回数据:%s  预期数据:%s'%(DataA, DataB))
...
def JsonList(ListA, ListB, loguser):
    try:
        loguser.debug('实响应数据: %s 预期数据:%s' %(ListA, ListB))
...
def JsonDict(DictA, DictB, loguser):
    try:
        loguser.debug('DictA: %s DictB:%s' %(DictA, DictB))
    ...
    # try:
        for key, value in jsonB.items():
            ...
            if (key in jsonA):
                ...
                loguser.debug('普通类型判断 返回数据：KEY:%s  VALUE:%s   参考数据：KEY:%s  VALUE:%s'%(key, str(jsonA[key]), key, value))
            else:                            # key不在返回数据里
...
def preset(test_host, table, row, log_user):
        log_user.info('请求接前执行第 %s 条用例' %row)
        row=int(row)
...
def request_core(ex_table, table_line, test_host, log_user):
        ...
        log_user.info("请求响应时间:%dms 响应状态:%d %s cookies: %s  sessionid: %s"% (response_time, rq.status_code, status_info, rq.cookies, GlobalConfiguration.jsessionid))
...
def function_core(ex_table, table_line, test_case, test_host, log_user):
        ...
        if way =='POST':
            ...
            log_user.debug('requests_line_num:%s response_line_num:%s  data_type:%s  flow_sign:%s' %(str(requests_line_num), str(response_line_num), data_type, flow_sign))
            if data_type == '1' or data_type == '2':
        ...
        if criteria == 'AssertEqual':                                                  # 断言方式相等
            ...
            if result != '1':                                            # 调用断言方式 返回1 正常  ，非1不正确
                log_user.info('响应数据匹配失败:\n响应数据: %s 参考响应数据 :%s' % (str(info_a), str(info_b)))
                log_user.info('执行结束：%s 接口序号：%s  接口名称：%s   执行失败  原因：响应数据匹配失败' %(test_case_name, table_line, test_name))
                return 0, info_a, info_b
            else:
                log_user.info('执行结束： %s 接口序号：%s  接口名称：%s   执行通过' %(test_case_name, table_line, test_name))
                log_user.info('------ 数据分析结束 ------')
